function renderHero() {
  ...
  if (!p) {
    console.warn('Hero data missing: window.personal is undefined');
    return;
  }
  ...
  subtitle.innerHTML = Array.isArray(p.subtitle) ? p.subtitle.join('<br />') : '';
  ...
  chips.innerHTML = Array.isArray(p.heroChips)
    ? p.heroChips.map((t) => `<span class="hero__chip" role="listitem">${t}</span>`).join('')
    : '';
    ...
    <div class="hero__card-inner">
      ...
      <div class="hero__card-achievements">
        ...
        <ul class="hero__card-achievement-list" role="list">
          ${Array.isArray(p.achievements) ? p.achievements.map((a) => `<li>${Icons.check} ${a}</li>`).join('') : ''}
        </ul>
      </div>
    </div>
  `;
